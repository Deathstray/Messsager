...
io.on('connection', (socket) => {
    ...
    require('./models/User').findByIdAndUpdate(userId, { isOnline: true, lastSeen: new Date() }).catch(() => {});
    ...
    socket.on('screenshare:start', async ({ chatId }) => {
        if (!chatId) return;
        ...
        try {
            ...
            (chat?.members || []).forEach(memberId => {
                const id = String(memberId);
                if (id !== userId) io.to(`user:${id}`).emit('screenshare:available', { hostId: userId, chatId, roomId });
            });
        } catch {}
    });
    ...
    socket.on('screenshare:join', ({ roomId }) => {
        if (!roomId) return;
        socket.join(roomId);
        const hostSocket = [...io.sockets.sockets.values()].find(s => s.rooms.has(roomId) && String(s.user?.id) !== userId);
        if (hostSocket) {
