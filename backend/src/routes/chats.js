...
function isCreator(chat, userId) {
    return String(chat.created_by) === String(userId);
}
    ...
    try {
        ...
        if (type === 'dm') {
            ...
            const populated = await chat.populate([
                { path: 'members', select: MEMBER_POP },
                { path: 'created_by', select: MEMBER_POP },
                { path: 'admins', select: MEMBER_POP },
            ]);
...
router.put('/:id/members', auth, async (req, res) => {
    try {
        ...
        if (chat.is_public) {
            if (!chat.members.map(String).includes(String(req.user.id))) return res.status(403).json({ error: 'Вы не состоите в группе' });
        } else {
            if (!isCreator(chat, req.user.id)) return res.status(403).json({ error: 'Только создатель может приглашать' });
        }
...
router.post('/:id/moderate', auth, async (req, res) => {
    try {
        ...
        if (String(chat.created_by) === userId) return res.status(400).json({ error: 'Нельзя модерировать создателя' });
        ...
        const populated = await Chat.findById(chat._id)
            ...
            .populate('admins', MEMBER_POP);
        chat.members.forEach(uid => req.app.get('io').to(`user:${String(uid)}`).emit('chat:updated', populated));
        req.app.get('io').to(`user:${userId}`).emit(
            action === 'mute' ? 'chat:updated' : 'chat:removed',
            action === 'mute' ? populated : { chatId: String(chat._id) }
        );
        res.json(populated);
    } catch (err) {
        console.error(err);
