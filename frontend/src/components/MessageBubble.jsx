...
import { IconTrash, IconImage, IconVideo, IconFile } from '../icons/Icons';
import { useTheme } from '../context/ThemeContext';
...
export default function MessageBubble({ msg, isGroup, myId, onDelete, onReact, onContextMenu }) {
    ...
    return (
        ...
        >
            {/* Имя в группе */}
            {!isMe && isGroup && (
                <span style={{ fontSize: 11, color: msg.from_user?.avatar_color || colors.accent, marginLeft: 8, marginBottom: 2, fontWeight: 700 }}>
            ...
            <div style={{ maxWidth: '68%', background: bubbleBg, color: bubbleColor, padding: '9px 13px', borderRadius: 16, wordBreak: 'break-word', boxShadow: isMe ? '0 2px 8px rgba(123,31,58,.25)' : '0 1px 4px rgba(0,0,0,.08)', borderBottomLeftRadius: isMe ? 16 : 4, borderBottomRightRadius: isMe ? 4 : 16 }}>
                ...
                {msg.reply_to && (
                    <div style={{ borderLeft: `3px solid ${accentColor}`, paddingLeft: 8, marginBottom: 6, background: isMe ? 'rgba(255,255,255,0.1)' : colors.accentLight, borderRadius: '0 6px 6px 0', padding: '4px 4px 4px 8px' }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: accentText }}>
                ...
                {msg.files?.map((f, i) => (
                    <div key={i} style={{ marginTop: msg.text ? 6 : 0 }}>
                        ...
                        {isVid(f.mimetype) && (
                            <video src={fileUrl(f.filename)} controls style={{ maxWidth: '100%', maxHeight: 280, borderRadius: 10, display: 'block' }} />
                        )}
                        {isAudio(f.mimetype) && (
                        ...
                        {!isImg(f.mimetype) && !isVid(f.mimetype) && !isAudio(f.mimetype) && (
                            <a href={fileUrl(f.filename)} download={f.original_name}
                                style={{ display: 'flex', alignItems: 'center', gap: 8, color: isMe ? '#fff' : colors.textAccent, textDecoration: 'none', background: isMe ? 'rgba(255,255,255,0.15)' : colors.accentLight, borderRadius: 8, padding: '7px 10px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, marginTop: 4 }}>
                    ...
                    {isMe && (
                        <button onClick={e => { e.stopPropagation(); onDelete && onDelete(msg._id); }}
                            style={{ background: 'none', border: 'none', cursor: 'pointer', opacity: 0.55, padding: 0, display:'flex', alignItems:'center' }}><IconTrash size={12} color='#fff'/></button>
                    )}
                </div>
            </div>
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 3, flexWrap: 'wrap', justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
                    {reactions.map(r => (
                        ...
                        <button key={r.emoji}
                            ...
                            style={{ background: colors.bgSidebar, border: `1px solid ${colors.border}`, borderRadius: 12, padding: '2px 7px', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3, boxShadow: '0 1px 3px rgba(0,0,0,.08)' }}>
                            {r.emoji}
